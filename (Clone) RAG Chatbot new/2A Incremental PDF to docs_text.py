...
# Directory path
directory_path = "/Volumes/dip-ragproject3/rag-project3/rag-chatbot3"
...
pdf_volume_path = "/Volumes/dip-ragproject3/rag-project3/rag-chatbot3"  # Specify the path to the PDF volume directory

# Get the list of already processed PDF files from the Delta table
processed_files = spark.sql(f"SELECT DISTINCT file_name FROM `dip-ragproject3`.docs_track").collect()
...
# MAGIC %sql
# MAGIC insert into `dip-ragproject3`.docs_text (text)
...
spark.sql("""
    INSERT INTO `dip-ragproject3`.docs_track
    SELECT * FROM temp_table
    WHERE NOT EXISTS (
        SELECT 1 FROM `dip-ragproject3`.docs_track
        WHERE temp_table.file_name = `dip-ragproject3`.docs_track.file_name
    )
""")
